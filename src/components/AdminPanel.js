...
export default function AdminPanel({ users, cfg, setCfg, team, userName, emailCfg = {} }) {
  ...
  const saveEmailCfg = async () => {
    ...
    await set(dbRef(db, 'emailConfig'), data);
    if (data.publicKey && window.emailjs) {
      try { window.emailjs.init({ publicKey: data.publicKey }); } catch (e) {}
    }
      ...
      <div className="card" style={{ marginBottom:20 }}>
        ...
        {userList.map(([uid, u]) => (
          <div key={uid} style={{ display:'grid', gridTemplateColumns:'1fr 130px 110px 130px', gap:10, padding:'12px 14px', borderBottom:'1px solid rgba(0,0,0,0.05)', alignItems:'center', background:!u.team ? 'rgba(196,125,14,0.03)' : 'transparent' }}>
            ...
            <div>{u.team
              ? <span style={{ fontSize:12, fontFamily:'JetBrains Mono,monospace', display:'flex', alignItems:'center', gap:5 }}><span style={{ width:8, height:8, borderRadius:'50%', background:TEAMS[u.team]?.color, display:'inline-block' }}/>{TEAMS[u.team]?.name}</span>
              : <span style={{ fontSize:12, color:'#c47d0e', fontWeight:500 }}>Unassigned</span>}
            </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, marginBottom:20 }}>
        ...
        <div className="card">
          ...
          <div style={{ fontSize:12, color:'#4a4a54', marginBottom:12, lineHeight:1.6 }}>
            When a PO is submitted, an <strong>email</strong> (via EmailJS) + <strong>WhatsApp</strong> message goes to you for approval.
          </div>
          <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
            ...
            <div>
              ...
              <div style={{ fontSize:11, color:'#7a7a88', marginTop:3 }}>With country code, no + or spaces</div>
            </div>
            <div>
